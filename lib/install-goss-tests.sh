...
if [ -f /etc/pit-release ]; then
    ...
    for ncn in $NCNS; do
        scp $CMS_TESTING_RPM $GOSS_SERVERS_RPM $ncn:/tmp/
        # shellcheck disable=SC2029
        ssh $ncn "rpm -Uvh --force /tmp/$(basename $CMS_TESTING_RPM) /tmp/$(basename $GOSS_SERVERS_RPM) && systemctl restart goss-servers"
    done
