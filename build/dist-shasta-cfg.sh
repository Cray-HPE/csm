...
repodir="${ROOTDIR}/vendor/stash.us.cray.com/scm/shasta-cfg/stable"

# Use the version of yq in SHASTA-CFG
shopt -s expand_aliases
alias yq="${repodir}/utils/bin/$(uname | awk '{print tolower($0)}')/yq"

set -x

# Copy shasta-cfg content ignoring anything in the ignore file.
rsync -rlE --safe-links --exclude-from="${repodir}/package/ignore" "${repodir}/" "${distdir}/"
...
# Replace with static secrets that have FIXME values
yq m -i "${distdir}/customizations.yaml" "${repodir}/package/static_secrets.yaml"
